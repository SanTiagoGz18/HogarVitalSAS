...
if (isset($_POST['modificado'])){
    if(isset($_POST['id_producto']) && ($_POST['cantidad_producto'])){
		...
		if(mysqli_num_rows($verificar_producto)<1){
			header("location:../../../Views/Html/Verifications/VerificationsProducts/no_existe_modificar_producto.html");
			exit();
		}

        if($_REQUEST['categoria'] == 'no'){
			header("location:../../../Views/Html/Verifications/VerificationsProducts/modificar_agregue_categoria.html");
			exit();
		}
        if($_REQUEST['marca'] == 'no'){
			header("location:../../../Views/Html/Verifications/VerificationsProducts/modificar_agregue_marca.html");
			exit();
		}
		if($_REQUEST['proveedor'] == 'no'){
			header("location:../../../Views/Html/Verifications/VerificationsProducts/modificar_agregue_proveedor.html");
			exit();
		}
		...
		if($res){
			...
			header("location:../../../Views/Html/Verifications/VerificationsProducts/correcto_modificar_productos.html");
			exit();
		}
